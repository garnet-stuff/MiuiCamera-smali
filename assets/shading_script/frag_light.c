...
in vec2 vTexCoord; //纹理坐标(更新在顶点着色器)
out vec4 outColor;
//函数介绍
//图层滤色融合
...
vec4 adjustBrightness(vec4 rgbdata, float percent) {
    float r = texture(text_dark, vec2(0.0, rgbdata.r), 0.0).r;
    float g = texture(text_dark, vec2(0.0, rgbdata.g), 0.0).g;
    float b = texture(text_dark, vec2(0.0, rgbdata.b), 0.0).b;
...
vec4 getFromColor(vec2 uv) {
    vec4 rgbdata = texture(text_filter, uv);
...
vec4 addVideoFilter1(vec4 color) {
    ...
    vec4 color1 = texture(text_filter, poss1, 0.0);
    vec4 color2 = texture(text_filter, poss2, 0.0);
...
vec4 lightColor(vec2 uv) {
    vec4 light_color = texture(text_light, uv, 0.0);
    return light_color;
}

//获取噪声素材的纹理rgba
vec4 noiseColor(vec2 uv) {
    vec4 noise_color = texture(text_noise, uv, 0.0);
    return noise_color;
}
